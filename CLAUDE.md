...
- `main.ts` configures `ActivityLog` with the least-privilege `LOG_DATABASE_URL` connection when infrastructure is enabled. Audit Trail writes remain strict and are awaited inside business operations.
- Application diagnostics are written to the console through `#project/logger`; credentials, cookies, authorization values, tokens, secrets, passwords, and passkey responses must never be persisted.
...
- The gateway does not persist request access rows. Domain Audit Trail events are emitted at their authoritative server boundary, not by the Angular client.
- Tests for a service cover logger wiring, redaction, and strict Audit Trail failure behavior.
