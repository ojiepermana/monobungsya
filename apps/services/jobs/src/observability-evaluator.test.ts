...
import type { ClickHouseSignalReader } from '#project/observability';
...
function stateFor(
  states: Map<string, StoredAlertState>,
  ruleId: string,
): StoredAlertState {
  for (const [key, state] of states) {
    if (key.startsWith(`${ruleId}|`)) return state;
  }
  throw new Error(`state not found for ${ruleId}`);
}
...
describe('ObservabilityAlertEvaluator', () => {
  ...
  it('marks metric alerts unknown when the ClickHouse reader cannot read', async () => {
    const { database, notificationQueries, states } = createEvaluatorDatabase();
    const registry = new JobRegistry();
    registry.registerContract(observabilityAlertNotificationContract);
    const reader = {
    ...
    } as unknown as ClickHouseSignalReader;
    const evaluator = new ObservabilityAlertEvaluator(
      database,
      registry,
      'benchmarks/alert-rules.json',
      undefined,
      { readMode: 'clickhouse', clickHouseReader: reader },
    );

    await evaluator.evaluate(new Date('2026-08-25T00:10:00Z'));

    expect([...states.values()]).not.toHaveLength(0);
    expect(
      [...states.values()].every((state) => state.status === 'unknown'),
    ).toBe(true);
    expect(notificationQueries).toHaveLength(0);
  });

  it('uses the probe-only reader for availability without admitting it for configured metric reads', async () => {
    const { database } = createEvaluatorDatabase();
    const registry = new JobRegistry();
    registry.registerContract(observabilityAlertNotificationContract);
    let publicReaderQueries = 0;
    let probeReaderQueries = 0;
    const publicReader = {
      async queryRows<Row extends object>(): Promise<Row[]> {
        publicReaderQueries += 1;
        return [];
      },
    } as unknown as ClickHouseSignalReader;
    const probeReader = {
      async queryRows<Row extends object>(): Promise<Row[]> {
        probeReaderQueries += 1;
        return [
          {
            server_version: '26.3.17.110',
            database_count: 1,
            disk_usage_percent: 20,
            schema_version_column_count: 4,
            table_count: 4,
          },
        ] as Row[];
      },
    ...
    const evaluator = new ObservabilityAlertEvaluator(
      ...
      {
        readMode: 'clickhouse',
        clickHouseReader: publicReader,
        clickHouseProbeReader: probeReader,
      },
    );

    await evaluator.evaluate(new Date('2026-08-25T00:10:00Z'));
    await evaluator.probeClickHouse(new Date('2026-08-25T00:10:00Z'));

    expect(publicReaderQueries).toBeGreaterThan(0);
    expect(probeReaderQueries).toBe(1);
  });

  it('persists ClickHouse availability and disk transitions with the required windows', async () => {
    const { database, notificationQueries, states } = createEvaluatorDatabase();
    const registry = new JobRegistry();
    registry.registerContract(observabilityAlertNotificationContract);
    let available = false;
    let diskUsagePercent = 70;
    const reader = {
      async queryRows<Row extends object>(query: string): Promise<Row[]> {
    ...
    const evaluator = new ObservabilityAlertEvaluator(
      ...
      'benchmarks/alert-rules.json',
      undefined,
      { readMode: 'clickhouse', clickHouseProbeReader: reader },
    );

    await evaluator.probeClickHouse(new Date('2026-08-25T00:00:00Z'));
    await evaluator.probeClickHouse(new Date('2026-08-25T00:00:30Z'));
    expect(
      stateFor(states, 'observability.clickhouse.availability').status,
    ).toBe('firing');

    available = true;
    const warningProbe = await evaluator.probeClickHouse(
      new Date('2026-08-25T00:01:00Z'),
    );
    expect(warningProbe).toEqual({
      available: true,
      diskUsagePercent: 70,
      blocksBackfill: false,
    });
    expect(
      stateFor(states, 'observability.clickhouse.disk.warning').status,
    ).toBe('firing');
    expect(
      stateFor(states, 'observability.clickhouse.availability').status,
    ).toBe('firing');
    ...
    diskUsagePercent = 80;
    const blockedProbe = await evaluator.probeClickHouse(
      new Date('2026-08-25T00:01:30Z'),
    );
    expect(blockedProbe.blocksBackfill).toBe(true);
    expect(
      stateFor(states, 'observability.clickhouse.disk.firing').status,
    ).toBe('firing');

    diskUsagePercent = 90;
    await evaluator.probeClickHouse(new Date('2026-08-25T00:02:00Z'));
    expect(
      stateFor(states, 'observability.clickhouse.disk.critical').status,
    ).toBe('firing');
    expect(
      stateFor(states, 'observability.clickhouse.availability').status,
    ).toBe('resolved');
    expect(
      stateFor(states, 'observability.clickhouse.availability')
        .consecutive_healthy_windows,
    ).toBe(3);
    expect(notificationQueries).toHaveLength(5);
  });

  it('treats a reachable but schema-incompatible ClickHouse as unavailable', async () => {
    const { database } = createEvaluatorDatabase();
    const registry = new JobRegistry();
    registry.registerContract(observabilityAlertNotificationContract);
    const reader = {
    ...
    const evaluator = new ObservabilityAlertEvaluator(
      ...
      'benchmarks/alert-rules.json',
      undefined,
      { readMode: 'postgres', clickHouseProbeReader: reader },
    );

    await expect(
      evaluator.probeClickHouse(new Date('2026-08-25T00:00:00Z')),
    ).resolves.toEqual({
      available: false,
      diskUsagePercent: 20,
      blocksBackfill: true,
