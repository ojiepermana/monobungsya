...
export function loadAuthEnv(
  ...
  const result: AuthEnvironment = {
    ...environment,
    SMTP_HOST: source.SMTP_HOST ?? "localhost",
    SMTP_PORT: parseNumber(source.SMTP_PORT, 1025, "SMTP_PORT"),
  ...
  if (
    ...
    throw new Error(
      "SMTP_USERNAME, SMTP_PASSWORD, and INTERNAL_AUTH_SIGNING_SECRET are required when auth infrastructure is enabled",
    );
  }
