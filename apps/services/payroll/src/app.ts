...
import { createPayrollRoute } from './modules/payroll/payroll.route';
...
export function createApp(environment: AppEnvironment = loadEnv('payroll')) {
  ...
  return new Elysia({ name: environment.serviceName })
    ...
    .use(createPayrollRoute(environment.serviceName))
