...
describe('api gateway', () => {
  ...
  it('forwards cursor based access log queries unchanged', async () => {
    const app = createApp(
      loadGatewayEnv({
        NODE_ENV: 'test',
        PORT: '3000',
    ...
    let upstreamRequest: Request | undefined;
    globalThis.fetch = Object.assign(
      fetchFor(['logs:log:read'], (request) => {
        upstreamRequest = request;
        return Response.json({
          data: [],
          prevCursor: null,
    ...
    try {
      const response = await app.handle(
        new Request(
          'http://localhost/api/v1/logs/access-logs?from=2026-08-20T00%3A00%3A00.000Z&to=2026-08-21T00%3A00%3A00.000Z&cursor=eyJ2IjoxfQ&event=access_granted',
          { headers: { cookie: 'project_session=session-value' } },
        ),
      );
      expect(response.status).toBe(200);
      expect(upstreamRequest?.url).toBe(
        'http://logs.internal/internal/logs/access-logs?from=2026-08-20T00%3A00%3A00.000Z&to=2026-08-21T00%3A00%3A00.000Z&cursor=eyJ2IjoxfQ&event=access_granted',
      );
    } finally {
      globalThis.fetch = originalFetch;
    }
  ...
  it('preserves 429 Retry-After for both cursor log routes', async () => {
    const app = createApp(
      loadGatewayEnv({
        NODE_ENV: 'test',
        PORT: '3000',
