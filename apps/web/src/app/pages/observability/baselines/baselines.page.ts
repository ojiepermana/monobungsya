...
@Component({
  ...
  template: `
    <Page variant="stacked" scroll="content" [appearance]="layout.appearance()" class="h-full min-h-0">
      ...
      <PageContent class="grid min-h-0 content-start gap-5 overflow-auto px-3 py-5"><section class="grid gap-2"><p class="text-sm text-muted-foreground">Promoted benchmark baselines are sorted by promotion time and baseline ID so every page has a stable bidirectional cursor.</p>@if (expiredNotice()) {<p class="border border-amber-500/40 bg-amber-500/10 p-3 text-sm" role="status">This baseline link has expired. The first page was loaded with your filters intact.</p>}@if (error()) {<p class="border border-destructive/40 bg-destructive/10 p-4 text-sm text-foreground" role="alert">{{ error() }}</p>}@if (storageWarning()) {<p class="border border-amber-500/40 bg-amber-500/10 p-3 text-sm" role="status">Baseline storage is a blind spot, not an empty result.</p>}</section>@if (loading()) {<p class="border border-border bg-muted p-5 text-sm" role="status">Loading baselines...</p>} @else if (rows().length === 0) {<p class="border border-border bg-muted p-5 text-sm text-muted-foreground">No promoted baselines match the current filters.</p>} @else {<div class="overflow-auto rounded-base border border-border bg-card"><table class="min-w-full text-left text-sm"><caption class="sr-only">Promoted benchmark baselines</caption><thead class="border-b border-border text-xs uppercase text-muted-foreground"><tr><th scope="col" class="px-4 py-3">Promoted</th><th scope="col" class="px-4 py-3">Scenario</th><th scope="col" class="px-4 py-3">Environment</th><th scope="col" class="px-4 py-3">Fixture</th><th scope="col" class="px-4 py-3">State</th></tr></thead><tbody>@for (baseline of rows(); track baseline.baselineId) {<tr class="border-b border-border align-top last:border-0"><td class="whitespace-nowrap px-4 py-3">{{ formatDate(baseline.promotedAt) }}</td><td class="px-4 py-3"><p class="font-medium">{{ baseline.scenarioId }} · {{ baseline.scenarioVersion }}</p><p class="font-mono text-xs text-muted-foreground">{{ baseline.baselineId }}</p></td><td class="px-4 py-3">{{ baseline.environment }}</td><td class="px-4 py-3">{{ baseline.fixtureVersion }}</td><td class="px-4 py-3"><span Badge [variant]="baseline.active ? 'default' : 'secondary'">{{ baseline.active ? 'Active' : 'Inactive' }}</span></td></tr>}</tbody></table></div>}</PageContent>
