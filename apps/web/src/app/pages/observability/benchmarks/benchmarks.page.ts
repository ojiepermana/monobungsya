...
@Component({
  ...
  template: `
    <Page variant="stacked" scroll="content" [appearance]="layout.appearance()" class="h-full min-h-0">
      ...
      <PageContent class="grid min-h-0 content-start gap-5 overflow-auto px-3 py-5"><section class="grid gap-2"><p class="text-sm text-muted-foreground">Benchmark runs are linkable evidence for performance decisions. Promoted baselines live in the separate Baselines signal.</p>@if (expiredNotice()) {<p class="border border-amber-500/40 bg-amber-500/10 p-3 text-sm" role="status">This benchmark link has expired. The first page was loaded with your filters intact.</p>}@if (error()) {<p class="border border-destructive/40 bg-destructive/10 p-4 text-sm text-foreground" role="alert">{{ error() }}</p>}@if (storageWarning()) {<p class="border border-amber-500/40 bg-amber-500/10 p-3 text-sm" role="status">Benchmark storage is a blind spot, not an empty result.</p>}</section>@if (loading()) {<p class="border border-border bg-muted p-5 text-sm" role="status">Loading benchmark runs...</p>} @else if (rows().length === 0) {<p class="border border-border bg-muted p-5 text-sm text-muted-foreground">No benchmark runs match the current filters.</p>} @else {<div class="overflow-auto rounded-base border border-border bg-card"><table class="min-w-full text-left text-sm"><caption class="sr-only">Benchmark runs</caption><thead class="border-b border-border text-xs uppercase text-muted-foreground"><tr><th scope="col" class="px-4 py-3">Started</th><th scope="col" class="px-4 py-3">Scenario</th><th scope="col" class="px-4 py-3">Status</th><th scope="col" class="px-4 py-3">Environment</th><th scope="col" class="px-4 py-3">Evidence</th></tr></thead><tbody>@for (run of rows(); track run.runId) {<tr class="border-b border-border align-top last:border-0"><td class="whitespace-nowrap px-4 py-3">{{ formatDate(run.startedAt) }}</td><td class="px-4 py-3"><a class="font-medium underline-offset-4 hover:underline" [routerLink]="['/observability/benchmarks', run.runId]">{{ run.scenarioId }} · {{ run.scenarioVersion }}</a><p class="font-mono text-xs text-muted-foreground">{{ run.runId }}</p></td><td class="px-4 py-3"><span Badge [variant]="run.status === 'failed' ? 'destructive' : run.status === 'completed' ? 'default' : 'secondary'">{{ run.status }}</span></td><td class="px-4 py-3">{{ run.environment }}<p class="text-xs text-muted-foreground">Bun {{ run.bunVersion }}</p></td><td class="px-4 py-3 text-xs text-muted-foreground">{{ run.completeness }} · {{ run.comparisonStatus ?? 'not compared' }}</td></tr>}</tbody></table></div>}</PageContent>
