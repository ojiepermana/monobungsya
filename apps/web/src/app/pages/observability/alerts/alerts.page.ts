...
@Component({
  ...
  template: `
    <Page variant="stacked" scroll="content" [appearance]="layout.appearance()" class="h-full min-h-0">
      ...
      <PageContent class="grid min-h-0 content-start gap-5 overflow-auto px-3 py-5"><section class="grid gap-2"><p class="text-sm text-muted-foreground">Alert rows preserve rule state and the evidence window that produced it. A resolved state remains visible for auditability.</p>@if (expiredNotice()) {<p class="border border-amber-500/40 bg-amber-500/10 p-3 text-sm" role="status">This alert link has expired. The first page was loaded with your filters intact.</p>}@if (error()) {<p class="border border-destructive/40 bg-destructive/10 p-4 text-sm text-foreground" role="alert">{{ error() }}</p>}@if (storageWarning()) {<p class="border border-amber-500/40 bg-amber-500/10 p-3 text-sm" role="status">Alert storage is a blind spot, not an empty result.</p>}</section>@if (loading()) {<p class="border border-border bg-muted p-5 text-sm" role="status">Loading alerts...</p>} @else if (rows().length === 0) {<p class="border border-border bg-muted p-5 text-sm text-muted-foreground">No alert states match the current filters.</p>} @else {<div class="overflow-auto rounded-base border border-border bg-card"><table class="min-w-full text-left text-sm"><caption class="sr-only">Runtime alert states</caption><thead class="border-b border-border text-xs uppercase text-muted-foreground"><tr><th scope="col" class="px-4 py-3">Rule</th><th scope="col" class="px-4 py-3">Service and resource</th><th scope="col" class="px-4 py-3">State</th><th scope="col" class="px-4 py-3">Breaches</th><th scope="col" class="px-4 py-3">Last evaluated</th></tr></thead><tbody>@for (alert of rows(); track alert.ruleId + alert.seriesFingerprint) {<tr class="border-b border-border align-top last:border-0"><td class="px-4 py-3"><a class="font-medium underline-offset-4 hover:underline" [routerLink]="['/observability/alerts', alert.ruleId]">{{ alert.title ?? alert.ruleId }}</a><p class="font-mono text-xs text-muted-foreground">{{ alert.ruleId }} · {{ alert.ruleVersion }}</p></td><td class="px-4 py-3">{{ alert.serviceName }}<p class="text-xs text-muted-foreground">{{ alert.resourceKind }} · {{ alert.resourceName }}</p></td><td class="px-4 py-3"><span Badge [variant]="alert.status === 'firing' ? 'destructive' : alert.status === 'resolved' ? 'default' : 'secondary'">{{ alert.status }}{{ alert.severity ? ' · ' + alert.severity : '' }}</span></td><td class="px-4 py-3">{{ alert.consecutiveBreachWindows }} windows<p class="text-xs text-muted-foreground">transition {{ alert.transitionSequence }}</p></td><td class="whitespace-nowrap px-4 py-3">{{ formatDate(alert.lastEvaluatedAt) }}</td></tr>}</tbody></table></div>}</PageContent>
