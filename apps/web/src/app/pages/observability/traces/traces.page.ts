    ...
    <Page variant="stacked" scroll="content" [appearance]="layout.appearance()" class="h-full min-h-0">
      ...
      <PageContent class="grid min-h-0 content-start overflow-auto">
        ...
        @if (loading()) {<p class="border border-border bg-muted p-5 text-sm text-muted-foreground" role="status">Loading traces...</p>} @else if (rows().length === 0) {<p class="border border-border bg-muted p-5 text-sm text-muted-foreground">No sampled traces match this window.</p>} @else {
          <table class="min-w-full rounded-base bg-card text-left text-xs"><caption class="sr-only">Sampled runtime traces</caption><thead class="sticky top-0 z-10 bg-card text-xs uppercase text-muted-foreground"><tr><th scope="col" class="bg-card px-4 py-3 shadow-[inset_0_-1px_0_0_var(--color-border)]">Started</th><th scope="col" class="bg-card px-4 py-3 shadow-[inset_0_-1px_0_0_var(--color-border)]">Service and resource</th><th scope="col" class="bg-card px-4 py-3 shadow-[inset_0_-1px_0_0_var(--color-border)]">Status</th><th scope="col" class="bg-card px-4 py-3 shadow-[inset_0_-1px_0_0_var(--color-border)]">Duration</th><th scope="col" class="bg-card px-4 py-3 shadow-[inset_0_-1px_0_0_var(--color-border)]">Evidence</th></tr></thead><tbody>@for (trace of rows(); track trace.traceId) {<tr class="border-b border-border align-top last:border-0"><td class="whitespace-nowrap px-4 py-3">{{ formatDate(trace.startedAt) }}</td><td class="px-4 py-3"><a class="font-medium text-foreground underline-offset-4 hover:underline" [routerLink]="['/observability/traces', trace.traceId]">{{ trace.serviceName }} · {{ trace.resourceName }}</a><p class="font-mono text-xs text-muted-foreground">{{ trace.traceId }}</p></td><td class="px-4 py-3"><span Badge [variant]="statusVariant(trace.status)">{{ trace.status }}</span></td><td class="whitespace-nowrap px-4 py-3">{{ Number(trace.durationMs).toFixed(2) }} ms</td><td class="px-4 py-3 text-xs text-muted-foreground">{{ formatCount(trace.spanCount) }} spans · {{ trace.complete ? trace.samplingReason : 'partial tree' }}</td></tr>}</tbody></table>
        }
      </PageContent>
