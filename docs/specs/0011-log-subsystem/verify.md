...
Runtime evidence was completed on 2026-08-24 against the real browser, PostgreSQL, gateway, services, process shutdown, foreign queue, and E2E flow. The focused suites also passed.
...
- [x] Open `/users`, then open Log Akses: one `GET /api/v1/users` row shows the verified actor, success, status 200, route, request id, and access time → AC-10, AC-11, AC-15
- [x] Search, filter, or change page on `/users`: each backend request adds one access row, while client navigation itself adds no `page_view` row → AC-10, AC-11
- [x] Log Akses displays and can search route name, path, method, status, request id, actor email, outcome, and failure reason → AC-15
* [x] Open `/users`, then narrow Log Akses to the trace shown on its session row: separate `/api/v1/auth/session` and `/api/v1/users` rows remain, both show client route `/users`, the trace source says `client supplied`, and no internal auth lookup appears → AC-17, AC-19
* [x] An authenticated `/api/v1/auth/session` row shows session id, state, and the effective permission count plus the verified actor; it shows no role, permission names, expiry values, cookie, token, or raw body → AC-16, AC-19, AC-20
* [x] An invalid session still returns `{ authenticated: false }` to the browser while an operator with `logs:log:read` can see its stable internal reason on the access row → AC-16, AC-20
...
- [x] `bun test apps/services/auth/src` → magic link and passkey success or failure plus logout produce correlated access events without storing tokens or credential responses → AC-12, AC-14
...
- [x] `bun run test:web` → the access pages render the extended fields, and the `/users` page still issues one initial users request → AC-11, AC-15
* [x] `bun test apps/services/auth/src` → session inspection distinguishes missing cookie, unknown session, revoked, absolute expiry, idle expiry, missing user, deleted user, blocked user, and suspended user with the specified precedence; every non active public result remains `{ authenticated: false }` → AC-16, AC-20
* [x] `bun test apps/services/auth/src` → the internal session observation contains only state and reason; it contains no role, permission list, or permission count → AC-16
* [x] `bun test apps/services/auth/src` → session state classification and sliding expiry refresh use one database decision at database `now()`; a session crossing expiry is never refreshed after being classified invalid → AC-16
...
* [x] `bun test apps/gateway/erp/src` → anonymous and invalid session results remain HTTP 200 access successes with their state in details, while malformed or unavailable auth responses have null details and the mapped failure → AC-16, AC-20
...
* [x] `bun run test:web` → one router navigation UUID is sent to the gateway origin for the guard and first page request, a later navigation gets another UUID, foreign origins receive no client context headers, and the access viewer can narrow to one trace → AC-17, AC-18, AC-19
* [x] Run the `/users` E2E flow → the public session row and users row have different request ids, one shared trace id, client route `/users`, and exactly one row each → AC-17, AC-19
- [x] Start a writer, enqueue application and access rows, send SIGTERM, and verify its local queue drains before the log database client closes or the configured timeout reports a warning → AC-8
- [x] Hold a gateway write queue open while reading through the logs service and verify the logs service cannot flush that foreign queue; release and flush the gateway queue, then verify the row becomes visible → AC-8
...
- [x] A public or failed auth request has null actor fields; a verified session supplies actor id, email, and session id → AC-10, AC-12
