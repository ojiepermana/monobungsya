...
* **AC-12**: List Application Log dan Access Log memakai keyset cursor dua arah serta mengembalikan `prevCursor`, `nextCursor`, jumlah row halaman ini, filter, options, dan `storageStatus` tanpa exact total. Page size default 100 dan dibatasi maksimal 100. Audit Trail tetap memakai page, total, dan PostgreSQL. Trace list tetap memakai page size 50 dan Metric tetap memakai batas 200 series.
