...
/**
 * Browser flow tests for the Audit Trail viewer, traced to spec logs/0001.
...
test.describe('log pages as an admin', () => {
  ...
  test('covers AC-9: a search reloads on page 1 and Clear Filters resets everything', async ({
    ...
    await search.fill('e2e seeded audit row');
    await expect(pageLabel(page, 1)).toBeVisible();
    await expect(
      page.getByRole('cell', { name: /e2e seeded audit row/ }).first(),
    ).toBeVisible();
